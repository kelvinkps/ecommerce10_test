<?php 
    ...
    require_once("model/DaoMenu.php");

	$obj_login = new Login();
	$obj_usuario = new DaoUsuario();
    $obj_menu = new DaoMenu();
    $obj_cliente = new DaoCliente();
    $obj_fornecedor = new DaoFornecedor();
    ...
    if(!$obj_login->usuarioEstaLogado()){
            ...
            $usuario = $obj_usuario->buscaUsuarioPorId($conexao, $_SESSION['id']);
            $menus = $obj_menu->buscaMenusEtiuId($conexao, $usuario['eusu_etiu_id']);
            if($usuario['eusu_etiu_id'] == 1){
                $cliente = $obj_cliente->buscaClientePorUsuario($conexao, $usuario['eusu_id']);
                $nome = utf8_encode($cliente['ecli_nome']);
            }else{
                $fornecedor = $obj_fornecedor->buscaFornecedorUsuario($conexao, $usuario['eusu_id']);
                $nome = utf8_encode($fornecedor['efo_razao_social']);
            }
            if($usuario == null){
                header("Location: login.php");
            }
                                    ...
                                    <ul class="nav navbar-nav pull-right">
                                        ...
                                        <li class="dropdown dropdown-user dropdown-dark">
                                            <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
                                                ...
                                                <span class="username username-hide-mobile"><?=$nome?></span>
                                            </a>
                                ...
                                <div class="hor-menu  ">
                                    <ul class="nav navbar-nav">
                                        <?php foreach($menus as $menu){?>
                                            ...
                                            <a href="<?=$menu['eme_url']?>"> <?=utf8_encode($menu['eme_nome'])?>
